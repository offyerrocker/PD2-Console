...
Navigation
	- CTRL + (LEFTARROW/RIGHTARROW) to move cursor to next space/special char in left/right direction (spaces? periods? commas? figure that out)
...
Console.scrollbar_held_data = false 
--[[example: 
	{
		offset_x = 1,
		offset_y = 123
...
Hooks:Add("BaseNetworkSessionOnLoadComplete","console_onloaded",function()  --called on event PlayerManager:_internal_load()
...
--	BeardLib:AddUpdater("command_console_update",Console.update,true)
...
function Console:GetEscBehavior() --unused
	return self.settings.esc_behavior
end

function Console:GetKeyboardRegion() --obsolete
	local region_key = {
		[1] = "us",
		[2] = "uk"
	}
...
function Console:AddBind(category,id,input_str,held)
...
function Console:cmd_state(subcommand,value) --set player state
...
--	self:Log("Mousemoved " .. tostring(x) .. "," .. tostring(y))
	...
	--character rect?
	local scrollbar_data = self:_scrollbar_held() 
	if y and scrollbar_data then 
		managers.mouse_pointer:set_pointer_image("grab") --arrow link hand grab
		local history = self._panel:child("command_history_frame"):child("command_history_panel")
		history:set_y(y - (scrollbar_data.y - scrollbar_data.start_y))
--		history:set_y(scrollbar_data.start_y + (y - scrollbar_data.y))
		self:refresh_scroll_handle()
	elseif self._panel:child("scroll_handle"):inside(x,y) then 
		managers.mouse_pointer:set_pointer_image("hand")
	elseif self._panel:child("input_bg"):inside(x,y) then 
		managers.mouse_pointer:set_pointer_image("link")
	else
		managers.mouse_pointer:set_pointer_image("arrow")
	end
	
--	self:SetTrackerValue("trackerb",tostring(x) .. "," .. tostring(y))
end

function Console:mouse_pressed(o,button,x,y)
	if button == Idstring("0") then
	
		if self._panel:child("scroll_handle"):inside(x,y) then 
			self:_set_scrollbar_held(y,self._panel:child("command_history_frame"):child("command_history_panel"):y())
		end
--		self:Log("press")
--		self:SetTrackerValue("trackerb",tostring(x) .. "," .. tostring(y))
	end
...
function Console:mouse_released(o,button,x,y)
	if button == Idstring("0") then
--		self:Log("release")
		self:_set_scrollbar_released()
	end
end
...
function Console:mouse_double_clicked(o,button,x,y)
	
end

function Console:_scrollbar_held()
	return self.scrollbar_held_data
end

function Console:_set_scrollbar_held(y,start_y)
	self.scrollbar_held_data = {y = y,start_y = y}
end

function Console:_set_scrollbar_released()
	self.scrollbar_held_data = false
end

function Console:enter_text(o,s)
...
--]]
	...
	local lbs = o:line_breaks()

	if #lbs > 1 then
		local s = lbs[2]
		local e = utf8.len(o:text())
...
--		text:set_text(self:string_excise(current,s,e,clipboard))
	...
	elseif k == Idstring("c") and ctrl_held then 
		Application:set_clipboard(string.sub(text:text(),text:selection()))
		if false then
			text:set_selection(0,current_len)
		end
...
function Console:GetCharList() --obsolete
...
function Console:BuildCharList(region) --obsolete and also stupid
	local characters = {
		us = {
...
--	Console._menu = managers.menu:register_menu("console_menu")
	...
	local console_master = Console._ws:panel()
	console_master:set_name("console_master")
	console_master:set_layer(69420)
	local console_base = console_master:panel({name = "console_base",layer = 1000,visible = false})
...
--	local orig_hud = managers.menu_component and managers.menu_component._fullscreen_ws --or managers.hud:script(PlayerBase.PLAYER_INFO_HUD_PD2).panel
	...
	console_w = console_w or console_base:w()
	console_h = console_h or console_base:h()
	...
	local debug_hud_base = console_master:panel({
		name = "debug_hud_base", --used for trackers
		layer = 999,
		visible = false --show by default TODO
	})
